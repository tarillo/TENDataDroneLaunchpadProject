...
class k_means{
   ...
   double getSumOfDistances();
    ...
    void modified_nearest_neighbor_distance(vector<vector<tuple<int,double,double>>> IndividualClusters);
...
void k_means::modified_nearest_neighbor_distance(vector<vector<tuple<int,double,double>>> IndividualClusters) {
    ...
    for(int c = 0; c < IndividualClusters.size(); ++c){
        ...
        for (int i = 1; i < cluster_size; ++i) {
            int next_tree = -1;
            int second_best_tree = -1;
            double bestDist = 100000;
            double secondBestDist = 100000;
